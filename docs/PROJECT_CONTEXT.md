...
hxue (薛煌) 的个人技术博客 / Obsidian 笔记公开站。用户确认第一版采用：
...
长期内容源是本地 Obsidian 笔记库 `D:\Document\Notes`。当前公开源目录是 `知识库`，通过 `sync-config.json` 增量同步到项目 `content/imported`。

开发或写博客时，新增技术博客、项目复盘、AI Coding 实战、面试路线等内容应先在 Obsidian 里落库，遵守 vault 的目录、双链、标签和 frontmatter 规则，再 dry-run 检查并同步到项目。项目内 `content/` 是发布镜像和构建源，不是长期写作入口；只有用户明确要求临时 hotfix 时才直接改项目内容。
...
- 阅读时间、目录、相关文章。
- 代码块复制按钮：支持 HTTPS Clipboard API 和 HTTP fallback，使用 Quartz SPA 的 `nav` / `addCleanup` 机制避免重复绑定。
- 阅读体验增强：阅读进度条、回到顶部、标题锚点、外链提示。
